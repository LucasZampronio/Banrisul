...
- Criar classe `TextAnalyzer` com métodos:
  - `ContarLinhas(string path)` - lê arquivo e conta quantas linhas existem
  - `ContarPalavras(string path)` - Lê arquivo e conta quantas palavras existem
  - `ContarLetras(string path)` - Lê arquivo e conta quantas letras existem
  - `GetTopWord(string path)` - lê o arquivo e retorna a palavra mais frequente
